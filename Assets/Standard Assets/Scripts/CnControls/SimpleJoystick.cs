...
namespace CnControls
{
	public class SimpleJoystick : MonoBehaviour, IDragHandler, IEventSystemHandler, IPointerUpHandler, IPointerDownHandler
	{
		...
		public virtual void OnDrag(PointerEventData eventData)
		{
			...
			Vector2 a = vector / magnitude;
			if (magnitude > MovementRange)
			{
				if (MoveBase && SnapsToFinger)
				{
					float d = vector.magnitude - MovementRange;
					Vector2 vector2 = a * d;
					_baseTransform.anchoredPosition += vector2;
					_intermediateStickPosition += vector2;
				}
				else
				{
					_stickTransform.anchoredPosition = _intermediateStickPosition + a * MovementRange;
				}
			}
			float value = Mathf.Clamp(vector.x * _oneOverMovementRange, -1f, 1f);
			float value2 = Mathf.Clamp(vector.y * _oneOverMovementRange, -1f, 1f);
