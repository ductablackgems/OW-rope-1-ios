...
public class QueueUvAnimation : MonoBehaviour
{
	...
	private IEnumerator UpdateTiling()
	{
		while (isVisible && allCount != count)
		{
			...
			Vector2 value = (!isFadeHandle) ? new Vector2((float)index / (float)ColumnsFadeIn - (float)(index / ColumnsFadeIn), 1f - (float)(index / ColumnsFadeIn) / (float)RowsFadeIn) : new Vector2((float)index / (float)ColumnsLoop - (float)(index / ColumnsLoop), 1f - (float)(index / ColumnsLoop) / (float)RowsLoop);
			if (!isFadeHandle)
			{
				GetComponent<Renderer>().material.SetTextureOffset("_MainTex", value);
				if (IsBump)
				{
					GetComponent<Renderer>().material.SetTextureOffset("_BumpMap", value);
				}
			}
			else
			{
