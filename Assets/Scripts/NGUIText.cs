...
public static class NGUIText
{
	...
	public static void Align(List<Vector3> verts, int indexOffset, float printedWidth, int elements = 4)
	{
		...
		{
			float num19 = (float)rectWidth - printedWidth;
			if (!(num19 < 0f))
			{
				int j = indexOffset;
		...
		{
			float num16 = ((float)rectWidth - printedWidth) * 0.5f;
			if (!(num16 < 0f))
			{
				int num17 = Mathf.RoundToInt((float)rectWidth - printedWidth);
				int num18 = Mathf.RoundToInt(rectWidth);
				bool flag = (num17 & 1) == 1;
				bool flag2 = (num18 & 1) == 1;
				if ((flag && !flag2) || (!flag && flag2))
				{
					num16 += 0.5f * fontScale;
				}
				int i = indexOffset;
		...
		for (; j < length; j++)
		{
			...
			if (c == '\n')
			{
				...
				if (wrapLineColors)
				{
					...
					{
						s.Append("[");
						s.Append(EncodeColor(mColors[l]));
						s.Append("]");
					}
				}
				flag = true;
			...
			if (encoding && ParseSymbol(text, ref j, mColors, premultiply, ref sub, ref bold, ref italic, ref underline, ref strike, ref ignoreColor))
			{
				if (num5 == num2 && useEllipsis && i < num7)
				{
					...
					if (num7 > i)
					{
						s.Append(text.Substring(i, num7 - i + 1));
					}
					if (num8 != 0)
					{
						s.Append("[/sub]");
			...
			float num11 = (sub == 0) ? fontScale : (fontScale * 0.75f);
			float num12;
			if (bMSymbol == null)
			{
				float glyphWidth = GetGlyphWidth(c, prev, num11);
			...
			{
				num12 = finalSpacingX + (float)bMSymbol.advance * num11;
			}
			if (sub != 0)
			{
			...
			if (flag4 && !flag3 && i < j)
			{
				...
				if (flag5 && useEllipsis && i < num7 && num4 < num3 && num4 > num13)
				{
					if (num7 > i)
					{
						s.Append(text.Substring(i, num7 - i + 1));
				...
				s.Append(text.Substring(i, num14 + 1));
				flag = false;
				i = j + 1;
			}
			...
			if (num4 > num13)
			{
				...
				if (wrapLineColors)
				{
					...
					for (int num19 = 0; num19 < mColors.size; num19++)
					{
						s.Append("[");
						s.Append(EncodeColor(mColors[num19]));
						s.Append("]");
					}
				}
				flag = true;
