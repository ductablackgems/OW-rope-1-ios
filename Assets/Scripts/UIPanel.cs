...
public class UIPanel : UIRect
{
	...
	private void UpdateWidgets()
	{
		...
		for (int count = widgets.Count; j < count; j++)
		{
			...
			if (uIWidget.UpdateTransform(frameCount) || mResized || (mHasMoved && !alwaysOnScreen))
			{
				...
				uIWidget.UpdateVisibility(visibleByAlpha, flag2 || alwaysOnScreen || (!hasCumulativeClipping && !uIWidget.hideIfOffScreen) || IsVisible(uIWidget));
			}
	...
	public UIDrawCall FindDrawCall(UIWidget w)
	{
		...
		{
			UIDrawCall uIDrawCall = drawCalls[i];
			int num = (i == 0) ? int.MinValue : (drawCalls[i - 1].depthEnd + 1);
			int num2 = (i + 1 == drawCalls.Count) ? int.MaxValue : (drawCalls[i + 1].depthStart - 1);
