...
public class UIGrid : UIWidgetContainer
{
	...
	public List<Transform> GetChildList()
	{
		...
		for (int i = 0; i < transform.childCount; i++)
		{
			...
			if (!hideInactive || ((bool)child && child.gameObject.activeSelf))
			{
				list.Add(child);
			}
		}
	...
	public static int SortHorizontal(Transform a, Transform b)
	{
		...
		return localPosition.x.CompareTo(b.localPosition.x);
	}

	public static int SortVertical(Transform a, Transform b)
	{
		Vector3 localPosition = b.localPosition;
		return localPosition.y.CompareTo(a.localPosition.y);
	}
	...
	protected virtual void ResetPosition(List<Transform> list)
	{
		...
		for (int count = list.Count; i < count; i++)
		{
			...
			if (animateSmoothly && Application.isPlaying && (pivot != 0 || Vector3.SqrMagnitude(transform2.localPosition - vector) >= 0.0001f))
			{
