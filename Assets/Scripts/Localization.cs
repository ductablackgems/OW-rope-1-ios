...
public static class Localization
{
	...
	public static string Get(string key, bool warnIfMissing = true)
	{
		...
		if (mLanguageIndex == -1)
		{
			...
			UnityEngine.Debug.LogWarning("Language not found: " + language);
		}
		string value;
		string[] value2;
	...
	public static void Set(string language, string key, string text)
	{
		...
		if (knownLanguages == null)
		{
			...
			knownLanguages = mLanguages;
		}
		int i = 0;
		for (int num = knownLanguages.Length; i < num; i++)
		{
			if (knownLanguages[i] == language)
			{
				if (!mDictionary.TryGetValue(key, out string[] value))
				{
					value = new string[knownLanguages.Length];
		...
		foreach (KeyValuePair<string, string[]> item in mDictionary)
		{
			...
			array[num2 - 1] = array[0];
			dictionary.Add(item.Key, array);
		}
		mDictionary = dictionary;
		if (!mDictionary.TryGetValue(key, out string[] value2))
		{
			value2 = new string[knownLanguages.Length];
