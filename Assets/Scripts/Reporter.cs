...
public class Reporter : MonoBehaviour
{
	...
	private Vector2 getDownPos()
	{
		...
		{
			downPos.x = UnityEngine.Input.mousePosition.x;
			downPos.y = UnityEngine.Input.mousePosition.y;
			return downPos;
		}
		return Vector2.zero;
	}
	...
	private IEnumerator readInfo()
	{
		...
		if (text.IndexOf("://") == -1)
		{
			...
			if (text2 == "")
			{
				text2 = Application.dataPath + "/StreamingAssets/";
			}
			url = Path.Combine(text2, text);
		}
		WWW www = new WWW(url);
		yield return www;
