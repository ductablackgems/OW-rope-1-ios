	...
	public class Bloom_FPS : MonoBehaviour
	{
		...
		public Shader shader
		{
			get
			{
				if (m_Shader == null)
				{
					m_Shader = Shader.Find("Hidden/Image Effects/Cinematic/Bloom_FPS");
				}
				return m_Shader;
			}
		}
		...
		public Material material
		{
			get
			{
				if (m_Material == null)
				{
					m_Material = ImageEffectHelper_FPS.CheckShaderAndCreateMaterial(shader);
				}
				return m_Material;
			}
		}
		...
		{
			if (!ImageEffectHelper_FPS.IsSupported(shader, needDepth: true, needHdr: false, this))
			{
				base.enabled = false;
			}
		}
