...
public class TweenLetters : UITweener
{
	...
	private void GetLetterDuration(int letterCount)
	{
		if (mCurrent.randomDurations)
		{
			...
			{
				mLetter[i].start = UnityEngine.Random.Range(0f, mCurrent.randomness.x * base.duration);
				float num = UnityEngine.Random.Range(mCurrent.randomness.y * base.duration, base.duration);
				mLetter[i].duration = num - mLetter[i].start;
			}
			return;
		}
		float num2 = base.duration / (float)letterCount;
		float num3 = 1f - mCurrent.overlap;
		float num4 = num2 * (float)letterCount * num3;
		float duration = ScaleRange(num2, num4 + num2 * mCurrent.overlap, base.duration);
		float num5 = 0f;
		for (int j = 0; j < mLetter.Length; j++)
		{
			int num6 = mLetterOrder[j];
			mLetter[num6].start = num5;
			mLetter[num6].duration = duration;
			num5 += mLetter[num6].duration * num3;
		}
	}
