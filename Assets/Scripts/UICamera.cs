...
public class UICamera : MonoBehaviour
{
	...
	public static GetKeyStateFunc GetKeyDown = (KeyCode key) => (key >= KeyCode.JoystickButton0 && ignoreControllerInput) ? false : UnityEngine.Input.GetKeyDown(key);

	public static GetKeyStateFunc GetKeyUp = (KeyCode key) => (key >= KeyCode.JoystickButton0 && ignoreControllerInput) ? false : UnityEngine.Input.GetKeyUp(key);

	public static GetKeyStateFunc GetKey = (KeyCode key) => (key >= KeyCode.JoystickButton0 && ignoreControllerInput) ? false : UnityEngine.Input.GetKey(key);

	public static GetAxisFunc GetAxis = (string axis) => ignoreControllerInput ? 0f : UnityEngine.Input.GetAxis(axis);
	...
	public static GetTouchDelegate GetTouch = delegate(int id, bool createIfMissing)
	{
		...
		int i = 0;
		for (int count2 = mTouchIDs.Count; i < count2; i++)
		{
			if (mTouchIDs[i] == id)
			{
	...
	public static KeyCode currentKey
	{
		...
		set
		{
			...
			{
				if (!InputUtils.HasCursorControl)
				{
	...
	public static bool isOverUI
	{
		get
		{
			...
			if (mLastOverCheck != frameCount)
			{
				...
				for (int j = 0; j < 3; j++)
				{
					if (IsPartOfUI(mMouse[j].current))
					{
						mLastOverResult = true;
						return mLastOverResult;
					}
			...
			if (uICamera.eventType == EventType.UI_3D)
			{
				...
				if (num > 1)
				{
					for (int j = 0; j < num; j++)
					{
						...
						if (component != null)
						{
							if (!component.isVisible || (component.hitCheck != null && !component.hitCheck(mRayHits[j].point)))
							{
								continue;
							}
						}
	...
	private void Awake()
	{
		...
		mMouse[0].pos = UnityEngine.Input.mousePosition;
		for (int i = 1; i < 3; i++)
		{
	...
	private void ProcessEvents()
	{
		...
		if (useMouse && mHover != null)
		{
			...
			if (currentScheme == ControlScheme.Mouse && showTooltips && mTooltipTime != 0f && !UIPopupList.isOpen && mMouse[0].dragged == null && (mTooltipTime < RealTime.time || GetKey(KeyCode.LeftShift) || GetKey(KeyCode.RightShift)))
			{
	...
	{
		int num = (GetInputTouchCount == null) ? UnityEngine.Input.touchCount : GetInputTouchCount();
		for (int i = 0; i < num; i++)
		{
			TouchPhase phase;
			int fingerId;
