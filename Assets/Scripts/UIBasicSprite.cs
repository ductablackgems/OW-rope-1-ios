...
public abstract class UIBasicSprite : UIWidget
{
	...
	private void SimpleFill(List<Vector3> verts, List<Vector2> uvs, List<Color> cols)
	{
		...
		{
			AddVertexColours(cols, ref color, 1, 1);
			AddVertexColours(cols, ref color, 1, 2);
			AddVertexColours(cols, ref color, 2, 2);
			AddVertexColours(cols, ref color, 2, 1);
		}
	}
		...
		for (int i = 0; i < 3; i++)
		{
			...
			for (int j = 0; j < 3; j++)
			{
				if (centerType != 0 || i != 1 || j != 1)
				{
					...
					{
						AddVertexColours(cols, ref color, i, j);
						AddVertexColours(cols, ref color, i, num2);
						AddVertexColours(cols, ref color, num, num2);
						AddVertexColours(cols, ref color, num, j);
					}
				}
			}
		}
