...
public static class NGUITools
{
	...
	public static T AddChild<T>(this GameObject parent) where T : Component
	{
		...
		if (!mTypeNames.TryGetValue(typeof(T), out string value) || value == null)
		{
			value = GetTypeName<T>();
	...
	{
		if (mWidgets.TryGetValue(id, out UIWidget value) && (bool)value)
		{
			return (T)value;
		}
		if (mRoot == null)
		{
			...
			for (int i = 0; i < UIRoot.list.Count; i++)
			{
				...
				if ((bool)uIRoot2)
				{
					...
					if ((bool)uICamera && uICamera.cachedCamera.orthographic)
					{
						x = uICamera;
						uIRoot = uIRoot2;
						break;
