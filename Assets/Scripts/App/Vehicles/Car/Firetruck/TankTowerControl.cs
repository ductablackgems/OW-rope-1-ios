...
namespace App.Vehicles.Car.Firetruck
{
	public class TankTowerControl : MonoBehaviour, ITankTowerControl
	{
		...
		public float cannonAngleOffset;
		...
		private void Awake()
		{
			_camera = ServiceLocator.GetGameObject("MainCamera").transform;
		}
		...
		protected void Update()
		{
			...
			{
				vector = new Vector3(_camera.eulerAngles.x + cameraRotator.eulerAngles.x + cannonAngleOffset, _camera.eulerAngles.y - cameraRotator.eulerAngles.y, 0f);
			}
			else
			{
				if (targetTransform != null)
				{
					virtualTarget.position = targetTransform.position;
				}
				vector = Quaternion.LookRotation(virtualTarget.localPosition - base.transform.localPosition).eulerAngles;
			}
			Quaternion to = Quaternion.Euler(initialEuler.x, vector.y, initialEuler.z);
			base.transform.localRotation = Quaternion.RotateTowards(base.transform.localRotation, to, Time.deltaTime * horizontalSpeed);
			Quaternion to2 = Quaternion.Euler(vector.x, initialCannonEuler.y, initialCannonEuler.z);
