	...
	private void UpdateWorldPosition()
	{
		...
		if (RandomMoveRadius > 0f)
		{
			...
			if (Target != null)
			{
				...
				float d = Vector3.Distance(t.position, targetT.position) / Vector3.Distance(startPosition, targetT.position);
				vector *= d;
			}
		}
		...
		if (!isCollided && !isOutDistance)
		{
			if (Target == null)
			{
				Vector3 point = (Vector3.forward + vector) * Speed * Time.deltaTime;
				b = t.localRotation * point;
				b2 = startQuaternion * point;
			}
			else
			{
				b2 = (b = ((targetT.position - t.position).normalized + vector) * Speed * Time.deltaTime);
			}
		}
		float magnitude = (t.localPosition + b - startPositionLocal).magnitude;
		UnityEngine.Debug.DrawRay(t.position, b2.normalized * (Distance - magnitude));
		RaycastHit hitInfo;
		if (!isCollided && Physics.Raycast(t.position, b2.normalized, out hitInfo, Distance, CollidesWith) && b.magnitude + 0.15f > hitInfo.distance)
		{
			isCollided = true;
