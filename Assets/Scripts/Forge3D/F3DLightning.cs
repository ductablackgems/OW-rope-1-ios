...
namespace Forge3D
{
	...
	public class F3DLightning : MonoBehaviour
	{
		...
		private void OnSpawned()
		{
			...
			if ((bool)F3DTime.time && Oscillate && Points > 0)
			{
		...
		private void Raycast()
		{
			...
			if (Physics.Raycast(ray, out hitPoint, MaxBeamLength, layerMask))
			{
				...
				ApplyForce(0.1f);
				if ((bool)DamagePrefab)
				{
					GameObject gameObject = UnityEngine.Object.Instantiate(DamagePrefab);
					if ((bool)gameObject)
					{
						gameObject.transform.position = hitPoint.point;
					}
				}
				if ((bool)rayImpact)
				{
