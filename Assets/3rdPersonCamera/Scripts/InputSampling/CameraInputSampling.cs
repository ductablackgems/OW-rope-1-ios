...
//#endif
        ...
        private void Update()
        {
            if (OWManager.instance != null && OWManager.instance.playerStop) return;
            ...
            if (cameraActions.MouseVisibilityToggle.WasPressedThisFrame())
            {
                ...
                freeLookInput.MouseHideMode = MouseCursorHelper.locked
                    ? FreeLookMouseHideMode.Always
                    : FreeLookMouseHideMode.OnInput;
            }
            ...
            if ((tmpInputFreeLook && !interfaceHovered && !waitForRelease) ||
                freeLookInput.MouseHideMode == FreeLookMouseHideMode.Always)
            ...
            freeLookInput.CameraInput = cameraActions.CameraAxis.ReadValue<Vector2>() * freeLookInput.MouseSensitivity *
                                        (freeLookInput.MouseInvertY ? new Vector2(1, -1) : new Vector2(1, 1));

            Vector2 controllerInput = cameraActions.ControllerCameraAxis.ReadValue<Vector2>();
            freeLookInput.CameraInput += controllerInput * freeLookInput.ControllerSensitivity *
                                         (freeLookInput.ControllerInvertY ? new Vector2(1, -1) : new Vector2(1, 1));
